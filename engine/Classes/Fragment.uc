...
simulated function HitWall (vector HitNormal, actor HitWall)
{
	...
	{
		if (FRand()<0.5) 
			PlaySound(ImpactSound, SLOT_SFX); //Change   PlaySound(ImpactSound, SLOT_None, 0.5+FRand()*0.5,, 300, 0.85+FRand()*0.3,true);
		else 
			PlaySound(AltImpactSound, SLOT_SFX); //Change  PlaySound(AltImpactSound, SLOT_None, 0.5+FRand()*0.5,, 300, 0.85+FRand()*0.3,true);
	}
}
